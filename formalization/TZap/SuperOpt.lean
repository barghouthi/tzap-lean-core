...
/-!
# SuperOpt Unitary Semantics

This file gives the formal circuit syntax a dense-matrix semantics for SuperOpt. Matrices use
SuperOpt's convention: rows are output basis states, columns are input basis states, and a gate
is applied on the left of the accumulated matrix. Consequently the head-first circuit `g :: C`
denotes `unitary C * unitaryGate g`.
...
/-- SuperOpt's dense matrix for a gate, defined independently entry-by-entry. This mirrors the
left-application kernels in `src/super_opt/matrix.rs` for the gate set present in the formal
syntax, while using the formalization's weighted-relation `Rz` convention. -/
...
/-- SuperOpt's dense unitary semantics. Starting at the identity and processing gates
...
/-- Exact entrywise correspondence between SuperOpt's unitary matrix and the existing
...
/-- Main semantic-equivalence theorem: after swapping the matrix indices, SuperOpt's unitary
