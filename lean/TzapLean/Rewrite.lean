...
/-- **The separation condition.** A gate that a later gate's rewrite does not claim misses
that rewrite's wires — so a rewrite's gates can be gathered at the first of them, crossing
only gates they commute with. -/
def Sep (supp : Nat → List Qubit) : List Tagged → Prop
  | [] => True
  | p :: rest =>
      (∀ w, p.2 ≠ some w → (∃ r ∈ rest, r.2 = some w) →
        ∀ q ∈ p.1.qubitsOf, q ∉ supp w) ∧ Sep supp rest
...
/-- `Sep` survives dropping gates: it only ever says less. -/
theorem Sep.unclaimed {supp : Nat → List Qubit} : ∀ {xs : List Tagged}, Sep supp xs →
    ∀ w, Sep supp (TzapLean.unclaimed w xs)
  | [], _, _ => trivial
  | (g, t) :: rest, h, w => by
      by_cases ht : t = some w
      · rw [ht, unclaimed_cons_self]
        exact Sep.unclaimed h.2 w
      · rw [unclaimed_cons_other _ _ ht]
        refine ⟨fun v hv hex => h.1 v hv ?_, Sep.unclaimed h.2 w⟩
        obtain ⟨r, hr, hrv⟩ := hex
        exact ⟨r, mem_unclaimed hr, hrv⟩
  ...
  | cons p rest ih =>
      ...
      · rw [claimedBy_cons_other _ _ ht, unclaimed_cons_other _ _ ht, untag_cons, untag_cons]
        have hcomm : Equivalent n m (claimedBy w rest ++ [g]) ([g] ++ claimedBy w rest) := by
          ...
          intro q hq hq'
          exact hsep.1 w ht ⟨(x, some w), hmem x hx, rfl⟩ q hq' ((honm x hx).1 q hq)
  ...
  | case3 g w rest ih =>
      ...
      · rw [applyAll, untag_cons]
        ...
        have step₃ : Equivalent n m (g :: (claimedBy w rest ++ untag (unclaimed w rest)))
            (g :: untag rest) :=
          Equivalent.append_left [g] (gather_equiv supp w rest hsep.2 hon.tail)
