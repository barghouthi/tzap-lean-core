...
#[test]
#[ignore = "manual release-mode runtime profile"]
fn profile_thousands_of_gates() {
    use std::time::Instant;

    for gate_count in [2_000, 5_000, 10_000] {
        let mut rng = TestRng(0x8bad_f00d_cafe_beef);
        let mut circuit = Circuit::new(64);
        for gate_index in 0..gate_count {
            let q = rng.next(circuit.num_qubits);
            let other = (q + 1 + rng.next(3)) % circuit.num_qubits;
            let gate = match rng.next(10) {
                ...
                3 => Gate::t(q),
                4 => Gate::tdg(q),
                5 | 6 => Gate::rz((gate_index + 1) as f64 / 997.0, q),
                7 | 8 => Gate::cnot {
                    control: q,
                    target: other,
                },
...
#[test]
#[ignore = "manual release-mode synthesis table profile"]
fn profile_synthesis_table() {
    build_profile_table();
}

#[test]
#[ignore = "manual deep synthesis-table build and serialization"]
fn build_deep_synthesis_table_artifact() {
    use std::time::Instant;

    let path = deep_table_path();
    let build_start = Instant::now();
    let table = UnitaryCircuitTable::build(SuperOptTableConfig {
        max_qubits: 4,
        max_gates: 8,
        max_entries_per_qubit: 1_000_000,
    })
    .unwrap();
    ...
    let expected_entries: Vec<_> = (1..=4)
        .map(|num_qubits| table.entry_count(num_qubits))
        .collect();
    let expected_depths: Vec<_> = (1..=4)
        .map(|num_qubits| table.completed_depth(num_qubits))
        .collect();
    drop(table);

    let load_start = Instant::now();
    let loaded = UnitaryCircuitTable::load(&path).unwrap();
    let load_elapsed = load_start.elapsed();
    assert_eq!(
        (1..=4)
            .map(|num_qubits| loaded.entry_count(num_qubits))
            .collect::<Vec<_>>(),
        expected_entries
    );
    assert_eq!(
        (1..=4)
            .map(|num_qubits| loaded.completed_depth(num_qubits))
            .collect::<Vec<_>>(),
        expected_depths
    );

    println!(
        "deep synthesis table: build {:.3} s, save {:.3} s, load {:.3} s, {:.1} MiB, entries {:?}, complete depths {:?}, saturated {:?}",
        ...
        expected_depths,
        (1..=4)
            .map(|num_qubits| loaded.is_saturated(num_qubits))
            .collect::<Vec<_>>(),
    );
}

fn deep_table_path() -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("experiments/unitary-circuit-table-q4-g8-e1m.bin")
}
...
fn build_profile_table() -> Arc<UnitaryCircuitTable> {
    use std::time::Instant;

    let start = Instant::now();
    let table = Arc::new(UnitaryCircuitTable::build(SuperOptTableConfig::default()).unwrap());
    println!(
        "synthesis table: {:.3} s, entries {:?}, complete depths {:?}, saturated {:?}",
        start.elapsed().as_secs_f64(),
        (1..=4)
            .map(|num_qubits| table.entry_count(num_qubits))
            .collect::<Vec<_>>(),
...
#[test]
#[ignore = "manual release-mode Cobble corpus profile"]
fn profile_cobble_circuits() {
    use std::time::Instant;
    ...
    for name in names {
        let path = format!(
            "{}/benchmarks/cobble/{name}.qasm",
            env!("CARGO_MANIFEST_DIR")
        );
        let source = std::fs::read_to_string(&path).unwrap();

        let parse_start = Instant::now();
        let circuit = crate::qasm::parse(&source).unwrap();
        ...
        println!(
            "{name:>24}: {:>7} gates -> {:>7} ({:>6} gates / {:>3} identities / {:>4} synth), T {:>6} -> {:>6}, parse {:>8.3} ms, run {:>9.3} ms, {:>7} windows, {:>7} cache hits / {:>7} misses",
            circuit.gates.len(),
            result.circuit.gates.len(),
            removed_gates,
            result.removed_subcircuits.len(),
            synthesized,
            crate::pass::count_t(&circuit),
            crate::pass::count_t(&result.circuit),
...
fn profile_default_pipeline_comparison(corpus: &str) {
    ...
    let mut total_baseline_gates = 0;
    let mut total_baseline_t = 0;
    let mut total_cancelled_gates = 0;
    let mut total_cancelled_t = 0;
    let mut total_default_gates = 0;
    let mut total_default_t = 0;
    let mut total_with_gates = 0;
    let mut total_with_t = 0;
    let mut total_cancel_time = std::time::Duration::ZERO;
    let mut total_default_phase_time = std::time::Duration::ZERO;
    let mut total_subcircuit_time = std::time::Duration::ZERO;
    let mut total_with_phase_time = std::time::Duration::ZERO;

    for path in paths {
        let name = path.file_stem().unwrap().to_string_lossy();
        let circuit = crate::qasm::parse(&std::fs::read_to_string(&path).unwrap()).unwrap();

        // Match main.rs: decompose Toffoli eagerly, count the baseline after.
        let circuit = if circuit.has_toffoli {
            Pass::run(&DecomposeToffoli, &circuit)
        } else {
            circuit
        };
        let baseline_gates = circuit.gates.len();
        let baseline_t = crate::pass::count_t(&circuit);

        let cancel_start = Instant::now();
        let cancelled = Pass::run(&CancelGates, &circuit);
        let cancel_elapsed = cancel_start.elapsed();

        let default_start = Instant::now();
        let default_result = Pass::run(&PhaseFoldRand, &cancelled);
        let default_elapsed = default_start.elapsed();

        let subcircuit_start = Instant::now();
        let rewritten = Pass::run(&subcircuit, &cancelled);
        let subcircuit_elapsed = subcircuit_start.elapsed();

        let with_phase_start = Instant::now();
        let with_result = Pass::run(&PhaseFoldRand, &rewritten);
        let with_phase_elapsed = with_phase_start.elapsed();

        total_baseline_gates += baseline_gates;
        total_baseline_t += baseline_t;
        total_cancelled_gates += cancelled.gates.len();
        total_cancelled_t += crate::pass::count_t(&cancelled);
        total_default_gates += default_result.gates.len();
        total_default_t += crate::pass::count_t(&default_result);
        total_with_gates += with_result.gates.len();
        total_with_t += crate::pass::count_t(&with_result);
        total_cancel_time += cancel_elapsed;
        total_default_phase_time += default_elapsed;
        total_subcircuit_time += subcircuit_elapsed;
        total_with_phase_time += with_phase_elapsed;

        println!(
            "{name:>24}: cancel {:>8} gates, {:>7} T | without {:>8} gates, {:>7} T ({:>9.3} ms phase) | with {:>8} gates, {:>7} T ({:>9.3} ms sub + {:>9.3} ms phase)",
            cancelled.gates.len(),
            crate::pass::count_t(&cancelled),
            default_result.gates.len(),
            crate::pass::count_t(&default_result),
            default_elapsed.as_secs_f64() * 1_000.0,
            with_result.gates.len(),
            crate::pass::count_t(&with_result),
            subcircuit_elapsed.as_secs_f64() * 1_000.0,
            with_phase_elapsed.as_secs_f64() * 1_000.0,
        );
    }

    println!(
        "TOTAL [{corpus}]: baseline {total_baseline_gates} gates, {total_baseline_t} T | cancel {total_cancelled_gates} gates, {total_cancelled_t} T ({:.3} s shared) | without {total_default_gates} gates, {total_default_t} T ({:.3} s phase) | with {total_with_gates} gates, {total_with_t} T ({:.3} s sub + {:.3} s phase)",
        total_cancel_time.as_secs_f64(),
        total_default_phase_time.as_secs_f64(),
        total_subcircuit_time.as_secs_f64(),
        total_with_phase_time.as_secs_f64(),
    );
}

#[test]
#[ignore = "manual release-mode Feynman default-pipeline comparison"]
fn profile_feynman_default_pipeline_with_and_without_subcircuit() {
    profile_default_pipeline_comparison("feynman");
}

#[test]
#[ignore = "manual release-mode Feynman fixpoint pipeline"]
fn profile_feynman_subcircuit_fixpoint() {
    use std::time::{Duration, Instant};

    use crate::cancel::CancelGates;
    use crate::phase_fold_rand::PhaseFoldRand;

    let directory = format!("{}/benchmarks/feynman", env!("CARGO_MANIFEST_DIR"));
    let mut paths: Vec<_> = std::fs::read_dir(directory)
        .unwrap()
        .map(|entry| entry.unwrap().path())
        .filter(|path| {
    ...
    let mut total_before_t = 0;
    let mut total_after_gates = 0;
    let mut total_after_t = 0;
    let mut total_time = Duration::ZERO;
    let mut round_histogram = std::collections::BTreeMap::<usize, usize>::new();

    for path in paths {
        let name = path.file_stem().unwrap().to_string_lossy();
        let circuit = crate::qasm::parse(&std::fs::read_to_string(&path).unwrap()).unwrap();
        let before_gates = circuit.gates.len();
        let before_t = crate::pass::count_t(&circuit);
        let start = Instant::now();
        let mut optimized = circuit.clone();
        let mut rounds = 0;
        loop {
            let gates_before_round = optimized.gates.len();
            optimized = Pass::run(&CancelGates, &optimized);
            optimized = Pass::run(&subcircuit, &optimized);
            optimized = Pass::run(&PhaseFoldRand, &optimized);
            rounds += 1;
            assert!(rounds <= 100, "{name} did not converge in 100 rounds");
            if optimized.gates.len() >= gates_before_round {
                break;
            }
        }
        let elapsed = start.elapsed();
        let after_t = crate::pass::count_t(&optimized);

        total_before_gates += before_gates;
        total_before_t += before_t;
        total_after_gates += optimized.gates.len();
        total_after_t += after_t;
        total_time += elapsed;
        *round_histogram.entry(rounds).or_default() += 1;
        println!(
            "{name:>24}: {before_gates:>8} -> {:>8} gates, {before_t:>7} -> {after_t:>7} T, {rounds:>2} rounds, {:>9.3} ms",
            optimized.gates.len(),
            elapsed.as_secs_f64() * 1_000.0,
        );
    }
    ...
    println!(
        "TOTAL [feynman fixpoint]: {total_before_gates} -> {total_after_gates} gates, {total_before_t} -> {total_after_t} T, {:.3} s, rounds {:?}",
...
#[test]
#[ignore = "manual release-mode SuperOpt window-size sweep on Feynman+Cobble"]
fn sweep_superopt_window_fixpoint_feynman_cobble() {
    use std::time::{Duration, Instant};

    use crate::phase_fold_rand::PhaseFoldRand;

    let max_window = std::env::var("SUPEROPT_SWEEP_MAX_WINDOW")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(16);
    let min_window = std::env::var("SUPEROPT_SWEEP_MIN_WINDOW")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(1);
    let max_rounds = std::env::var("SUPEROPT_SWEEP_MAX_ROUNDS")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(1_000);
    let stable_after = std::env::var("SUPEROPT_SWEEP_STABLE_AFTER")
        .ok()
        .and_then(|value| value.parse::<usize>().ok())
        .unwrap_or(4);
    ...
    for corpus in ["feynman", "cobble"] {
        ...
        for path in paths {
            ...
            let circuit = crate::qasm::parse(&std::fs::read_to_string(&path).unwrap())
                .unwrap_or_else(|error| panic!("failed to parse {path:?}: {error}"));
            circuits.push((format!("{corpus}/{name}"), circuit));
        }
    }

    let table = load_optimizer_profile_table();
    let baseline_gates: usize = circuits
        .iter()
        .map(|(_, circuit)| circuit.gates.len())
        .sum();
    let baseline_t: usize = circuits
        .iter()
        .map(|(_, circuit)| crate::pass::count_t(circuit))
        .sum();

    let mut best_gates = usize::MAX;
    let mut best_t = usize::MAX;
    let mut last_improving_window = 0;
    let mut stable_windows = 0;

    println!(
        "WINDOW SWEEP [PhaseFoldRand -> SuperOpt fixpoint, Feynman+Cobble]: {} circuits, baseline {baseline_gates} gates, {baseline_t} T",
        circuits.len()
    );
    println!(
        "window,total_gates,total_t,gate_delta,t_delta,rounds_hist,total_pass_seconds,improved"
    );
    ...
    for window_gates in min_window..=max_window {
        ...
        for (name, circuit) in &circuits {
            ...
            loop {
                let gates_before_round = optimized.gates.len();
                optimized = Pass::run(&PhaseFoldRand, &optimized);
                optimized = Pass::run(&superopt, &optimized);
                rounds += 1;
                assert!(
                    rounds <= max_rounds,
                    "{name} did not converge in {max_rounds} rounds at window {window_gates}"
                );
                if optimized.gates.len() >= gates_before_round {
                    break;
                }
            ...
            total_gates += optimized.gates.len();
            total_t += crate::pass::count_t(&optimized);
            *round_histogram.entry(rounds).or_default() += 1;
        }

        let improved = total_gates < best_gates || (total_gates == best_gates && total_t < best_t);
        if improved {
            best_gates = total_gates;
            best_t = total_t;
            last_improving_window = window_gates;
            stable_windows = 0;
        } else {
            stable_windows += 1;
        }
        ...
        println!(
            "{window_gates},{total_gates},{total_t},{},{},{:?},{:.3},{improved}",
            baseline_gates as isize - total_gates as isize,
            baseline_t as isize - total_t as isize,
            round_histogram,
            total_time.as_secs_f64(),
        );

        if window_gates >= 8 && stable_windows >= stable_after {
            println!(
                "plateau after window {last_improving_window}: no improvement for {stable_windows} consecutive windows through {window_gates}"
            );
            break;
        }
    }
...
#[test]
#[ignore = "manual release-mode SuperOpt synthesis-table size sweep on Feynman+Cobble"]
fn sweep_superopt_table_size_fixpoint_feynman_cobble() {
    use std::time::{Duration, Instant};

    use crate::phase_fold_rand::PhaseFoldRand;

    let entry_caps = std::env::var("SUPEROPT_ENTRY_CAPS")
        .unwrap_or_else(|_| "1000000,2000000,4000000".to_owned())
        .split(',')
        .map(|value| {
            value
                .trim()
                .parse::<usize>()
                .unwrap_or_else(|error| panic!("invalid SUPEROPT_ENTRY_CAPS value: {error}"))
        })
        .collect::<Vec<_>>();
    let max_rounds = std::env::var("SUPEROPT_SWEEP_MAX_ROUNDS")
        .ok()
    ...
    println!(
        "TABLE SIZE SWEEP [window=8, PhaseFoldRand -> SuperOpt fixpoint, Feynman+Cobble]: {} circuits, baseline {baseline_gates} gates, {baseline_t} T",
        circuits.len()
    );
    println!(
        "entry_cap,entries,complete_depths,saturated,build_seconds,total_gates,total_t,gate_delta,t_delta,rounds_hist,total_pass_seconds,improved"
    );
    ...
    for entry_cap in entry_caps {
        let default_config = SuperOptTableConfig::default();
        let table_config = SuperOptTableConfig {
            max_entries_per_qubit: entry_cap,
            ..default_config
        };
        let build_start = Instant::now();
        let table = Arc::new(UnitaryCircuitTable::build(table_config).unwrap());
        let build_elapsed = build_start.elapsed();
        let entries = (1..=table_config.max_qubits)
            .map(|num_qubits| table.entry_count(num_qubits))
            .collect::<Vec<_>>();
        let complete_depths = (1..=table_config.max_qubits)
            .map(|num_qubits| table.completed_depth(num_qubits))
            .collect::<Vec<_>>();
        let saturated = (1..=table_config.max_qubits)
            .map(|num_qubits| table.is_saturated(num_qubits))
            .collect::<Vec<_>>();
        ...
        for (name, circuit) in &circuits {
            ...
            loop {
                ...
                assert!(
                    rounds <= max_rounds,
                    "{name} did not converge in {max_rounds} rounds at entry cap {entry_cap}"
                );
                if optimized.gates.len() >= gates_before_round {
                    break;
                }
        ...
        println!(
            "{entry_cap},{entries:?},{complete_depths:?},{saturated:?},{:.3},{total_gates},{total_t},{},{},{:?},{:.3},{improved}",
            build_elapsed.as_secs_f64(),
            baseline_gates as isize - total_gates as isize,
            baseline_t as isize - total_t as isize,
            round_histogram,
...
#[test]
#[ignore = "manual release-mode Cobble default-pipeline comparison"]
fn profile_cobble_default_pipeline_with_and_without_subcircuit() {
    profile_default_pipeline_comparison("cobble");
}

#[test]
#[ignore = "manual release-mode Feynman suite profile"]
fn profile_feynman_circuits() {
    use std::time::Instant;
    ...
    for path in paths {
        ...
        let synthesized = result
            ...
            .filter(|rewrite| !rewrite.replacement.is_empty())
            .count();
        let t_before = crate::pass::count_t(&circuit);
        let t_after = crate::pass::count_t(&result.circuit);

        total_gates += circuit.gates.len();
        total_removed_gates += removed_gates;
        total_identities += result.removed_subcircuits.len();
        total_synthesized += synthesized;
        total_t_before += t_before;
        total_t_after += t_after;
        total_windows += result.cache_hits + result.cache_misses;
        total_cache_hits += result.cache_hits;
        total_cache_misses += result.cache_misses;
        total_parse += parse_elapsed;
        total_run += run_elapsed;

        println!(
            "{name:>24}: {:>8} gates -> {:>8} ({:>7} gates / {:>5} identities / {:>5} synth), T {:>6} -> {:>6}, parse {:>9.3} ms, run {:>10.3} ms, {:>8} windows, {:>8} cache hits / {:>8} misses",
            circuit.gates.len(),
            result.circuit.gates.len(),
            removed_gates,
            result.removed_subcircuits.len(),
    ...
    println!(
        "TOTAL: {total_gates} gates, {total_removed_gates} gates / {total_identities} identities / {total_synthesized} synth rewrites, T {total_t_before} -> {total_t_after}, parse {:.3} ms, run {:.3} ms, {total_windows} windows, {total_cache_hits} cache hits / {total_cache_misses} misses",
        total_parse.as_secs_f64() * 1_000.0,
        total_run.as_secs_f64() * 1_000.0,
    );
}
